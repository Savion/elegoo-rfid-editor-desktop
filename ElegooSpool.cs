...
namespace RFIDEditor
{
    public class ElegooSpool
    {
        ...
        //Unsure, testing. Either this OR 0x9 0xC6 is the production date, but doesn't match the firmware.
        public string ProductionDate
        {
            get
            {
                return RawData[0x60].ToString("X2") + RawData[0x61].ToString("X2");
            }
            set
            {
                if (value.Length == 4
                    && byte.TryParse(value.Substring(0, 2), System.Globalization.NumberStyles.HexNumber, null, out byte yy)
                    && byte.TryParse(value.Substring(2, 2), System.Globalization.NumberStyles.HexNumber, null, out byte mm))
                {
                    RawData[0x60] = yy;
                    RawData[0x61] = mm;
                }
